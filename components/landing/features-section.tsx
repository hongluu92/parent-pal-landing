...
function FeatureCard({
  ...
  const handleMouseMove = (e: React.MouseEvent<HTMLDivElement>) => {
    ...
    gsap.to(card, {
      ...
      boxShadow: isSpecialAI
        ? "0 25px 55px rgba(139, 92, 246, 0.18)"
        : "0 22px 48px rgba(30, 27, 75, 0.08)",
      ease: "power2.out",
      duration: 0.3,
    });
  };
  ...
  return (
    <article
      ...
      className={`feature-card group relative overflow-hidden rounded-[32px] border p-8 text-[#1E1B4B] shadow-[0_10px_30px_rgba(30, 27, 75, 0.02)] transition-all duration-300 ${bgColor} ${borderColor} ${className} ${isSpecialAI ? 'shadow-[0_15px_35px_rgba(139,92,246,0.10)] border-[#C3B5FF]' : ''
        }`}
    >
      {isLarge ? (
        <div className="grid gap-6 items-center lg:grid-cols-[1.2fr_1fr] w-full h-full">
          <div>
          ...
          {visual && (
            <div className="relative flex items-center justify-center mt-6 lg:mt-0 pointer-events-none">
              {visual}
            </div>
          )}
        </div>
  ...
  const bentoFeatures: Array<{
    ...
    isSpecialAI?: boolean;
  }> = [
      {
        ...
        bgColor: 'bg-gradient-to-br from-[#F5F3FF] via-[#EFEAFF] to-[#E5DEFF]',
        borderColor: 'border-[#C3B5FF]',
        isSpecialAI: true,
        visual: (
          <div className="relative h-36 w-48 overflow-hidden rounded-2xl border border-purple-200/80 bg-white p-3 shadow-[0_12px_30px_rgba(139,92,246,0.08)] flex flex-col justify-between transform rotate-[3deg] group-hover:rotate-0 transition-transform duration-300">
            ...
            <div className="flex-1 my-2 flex flex-col justify-center bg-purple-50/50 rounded-xl p-2 border border-dashed border-purple-100">
      ...
      },
      {
        ...
        bgColor: 'bg-[#FFF7ED]/70 backdrop-blur-sm',
      ...
      },
      {
        ...
        className: 'lg:col-span-1',
        bgColor: 'bg-[#EEF2FF]/70 backdrop-blur-sm',
      ...
      },
      {
        ...
        className: 'lg:col-span-2',
        bgColor: 'bg-[#F0FDF4]/70 backdrop-blur-sm',
        borderColor: 'border-[#DCFCE7]',
        visual: (
          // Polaroid sinh động chi tiết với hình vẽ chiếc giường nhỏ xinh bằng CSS
          <div className="relative bg-white p-2.5 pb-6 rounded-lg shadow-lg rotate-[4deg] border border-emerald-100 w-40 mx-auto group-hover:rotate-[-2deg] transition-transform duration-300">
      ...
      },
      {
        ...
        className: 'lg:col-span-1',
        bgColor: 'bg-[#FAF8F5]/80 backdrop-blur-sm',
      ...
      },
      {
        ...
        className: 'lg:col-span-2',
        bgColor: 'bg-[#FEF3C7]/60 backdrop-blur-sm',
    ...
    <section id="features" ref={containerRef} className="relative isolate overflow-hidden bg-[#FAF8F5] px-5 py-24 sm:px-8 border-t border-[#4F46E5]/10">
      ...
      <div className="relative mx-auto max-w-5xl">
        <div className="max-w-3xl mb-16 text-center sm:text-left">
          <h2 className="text-3xl font-black sm:text-4xl -tracking-[0.02em] text-[#1E1B4B]">
        ...
        <div className="grid gap-6 grid-cols-1 md:grid-cols-2 lg:grid-cols-3">
          {bentoFeatures.map(({ icon, title, body, stat, className, bgColor, borderColor, visual, isSpecialAI }, index) => (
            <FeatureCard
              key={title}
              icon={icon}
              title={title}
